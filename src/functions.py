...
def launch_simulation(m, N, L, h, eps,
                      eps_transport, time_step, nb_iter_max, c, 
    ...
    U_1 = U_0
    list_of_E, list_of_U = [energy_init], [[U_0, 0]]

    step_function_1 = get_AC_Wasserstein_explicit(U_0, m, time_step, eps, 
                                                  eps_transport, L, c_W=c)

    nb_prints = 5 # display_timelapse hardcodes a 2x3 array
    mod = nb_iter_max/nb_prints
    start = time.time()
    init = None
    i = 0
    prev_E_1, E_1 = 0, -1
    while i < nb_iter_max and (not np.isclose(E_1, prev_E_1,rtol=1e-8)):
        prev_E1 = E_1
        U_1, E_1, extra = step_function_1(U_1,init)
        init = extra
        list_of_E.append(E_1)
        if i % mod == 0 and i!=0:
            list_of_U.append([U_1, i])
            if verbose:
                print("i=",i, " E=", E_1)
        i += 1
    ...
    dic_param ={'shape': shape, 'c': c, 'N': N, 'nb_iter_max': nb_iter_max, 
                'm': m, 'L': L,'h': h, 'eps': eps, 
...
def display_figure(U_0, U_1, list_of_E, dic_param, cmap):
    ...
    table.scale(1, 1.5)
    ax_param.axis("off")
    ax_param.set_title('Parameters value')
    ...
    ax_nrg.set_title("Energy = f°($n_{iter})$")
...
def display_timelapse(U_0, U_1, list_of_E, dic_param, cmap, list_of_U):
    ...
    num_lin = 2
    num_col = 3
    gs = fig.add_gridspec(num_lin, num_col)
    ...
    table.scale(1, 0.7)
    ax_param.axis("off")
    ax_param.set_title('Parameters value', y=0)

    
    for idx, U in enumerate(list_of_U):
        ax = fig.add_subplot(gs[(idx+1)//num_col, (idx+1)%num_col])
        im = ax.imshow(U[0], cmap=cmap)
        ax.axis('off')

        if idx==0:
            x0, y0 = ax.get_position().x0, ax.get_position().y0
            height = ax.get_position().height
            cbar_height = 0.4*height
            center = y0 + height/2
            bottom = center - cbar_height/2
            ax_cbar= fig.add_axes([x0 + 0.02, bottom, 0.01, cbar_height])
            cbar = fig.colorbar(im, cax=ax_cbar)
            ax.set_title('$n_{iter}$ = '+str(U[1]), y=-0.01)
